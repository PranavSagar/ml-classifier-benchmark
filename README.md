...
| **Random Forest (Ensemble)** | 0.8599 | **0.8113** | 0.4201 | 0.6401 | **0.5073** | **0.4426** |
...
| **Decision Tree** | The widest net: best recall of all five (0.6573) but the second-worst precision (0.3476), so roughly two of every three customers it flags will decline. Depth had to be capped at 8: unconstrained, it hit **0.9946 training accuracy and collapsed to 0.6139 test AUC** — memorising the training set cost it 18 points of AUC against the capped version's 0.7993. Matches logistic regression on AUC but with visibly more variance, which is precisely what the ensemble fixes. |
...
| **Random Forest (Ensemble)** | Best on every metric that accounts for both classes: AUC 0.8113, F1 0.5073, MCC 0.4426. It keeps almost all of the single tree's recall (0.6401 vs 0.6573) while lifting precision from 0.3476 to 0.4201 — bagging removes the single tree's variance without giving up its sensitivity. Note its accuracy (0.8599) is *lower* than kNN's, which is the whole point. |
| **Overall winner for your dataset?** | **Random Forest.** On an 11%-positive target, accuracy is the wrong yardstick — it crowns kNN, which misses 73% of the subscribers the campaign exists to find. Ranking on MCC and AUC, which are both sensitive to performance on the minority class, puts Random Forest first, and it wins F1 as well. Logistic regression is the honourable mention: 0.04 behind on MCC for a fraction of the inference cost, and worth choosing if interpretability outranks the last few points. |
...
0.6401 to 0.8039 while precision falls from 0.4201 to 0.2058 — the right trade if a missed
...
Python 3.11. The five fitted pipelines total ~8 MB and are committed to the repo, so the
